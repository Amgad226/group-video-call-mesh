...
const Room = () => {
  ...
  function callUser(userID, socket_id, clientStream) {
    ...
    console.log("clientStream ref", clientStreamRef.current.getTracks());
    console.log("clientStream", clientStream.getTracks());
  ...
  function handleRecieveCall(incoming) {
    ...
    const existPeer = peersRef.current.find(
      (ref) => ref.peerID === incoming.callerID
    );
    if (existPeer) {
      const newPeers = peersRef.current.filter(
        (peer) => peer.peerID !== incoming.callerID
      );
